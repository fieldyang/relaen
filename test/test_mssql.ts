...
import { Shop } from "./entity_mssql/shop";
import { UserInfo } from "./entity_mssql/userinfo";
...
async function addShop() {
    ...
    //设置外键
    shop.setUserInfoForOwnerId(new UserInfo(1));
    await shop.save();
}
...
async function native() {
    ...
    let sql = "select t1.*,t2.* from t_shop t1,t_user t2 where t1.manager_id = t2.user_id order by shop_id"; // mssql
